#!/usr/bin/env python3
"""Fetch a Figma node and persist the REST payload under <output>/figma-data for leon-figma."""
...
REQUEST_PLAN_NAME = "request-plan.json"
...
def read_env_alias(canonical_name: str) -> tuple[bool, str | None, str | None]:
    normalized_target = normalize_env_key(canonical_name)
    for key, value in os.environ.items():
        if normalize_env_key(key) == normalized_target:
            return True, value, key
    return False, None, None


def resolve_token_value(cli_value: str | None) -> str | None:
    if cli_value is not None:
        return cli_value

    found, value, _ = read_env_alias("figma_token")
    if not found:
        return None
    return value


def resolve_proxy_value(cli_value: str | None) -> tuple[bool, str | None]:
    if cli_value is not None:
        return True, cli_value

    found, value, _ = read_env_alias("figma_proxy")
    if found:
        return True, value
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--token",
        help=(
            "Figma access token. Defaults to any environment variable whose name matches "
            "figma_token case-insensitively and separator-insensitively, such as FIGMA_TOKEN, "
            "figma_token, or figmaToken."
        ),
    )
    parser.add_argument(
        "--figma-proxy",
        ...
        help=(
            ...
            "Defaults to any environment variable whose name matches figma_proxy "
            "case-insensitively and separator-insensitively, such as FIGMA_PROXY, "
            "figma_proxy, or figmaProxy."
        ),
    )
    parser.add_argument(
        "--node-id",
...
def discover_local_figma_artifacts(output_root: Path) -> list[str]:
    artifacts: list[str] = []
    ...
    for path in output_root.rglob("*"):
        if not path.is_file():
            continue
        if "figma-data" not in path.parts:
            continue
        if path.name == REQUEST_PLAN_NAME:
            continue
...
def main() -> None:
    ...
    existing_local_artifacts = discover_local_figma_artifacts(output_root)
    ...
    if existing_local_artifacts and not args.force_refresh:
        fail(
            "Local persisted Figma artifacts already exist under the output path, "
            "but no reusable figma-api-export.json with SVG readiness metadata was found. "
            "Refusing to call the Figma REST API automatically. "
            "Inspect or repair the local artifacts, or rerun with --force-refresh if you explicitly want a fresh fetch. "
            f"Found artifacts: {existing_local_artifacts}"
        )

    token = resolve_token_value(args.token)
    if not token:
        fail(
            "Missing Figma token. Pass --token, or set an environment variable whose name "
            "matches figma_token case-insensitively, such as FIGMA_TOKEN, figma_token, or figmaToken."
        )

    proxy_found, figma_proxy = resolve_proxy_value(args.figma_proxy)
    if not proxy_found:
        fail(
            "Missing Figma proxy setting. Pass --figma-proxy, set an environment variable whose "
            "name matches figma_proxy case-insensitively, such as FIGMA_PROXY, figma_proxy, or "
            "figmaProxy, or explicitly pass --figma-proxy \"\" for direct access."
        )

    headers = build_headers(token)
    opener = build_url_opener(figma_proxy)
    ...
    export_payload = {
        "source": {
            ...
            "asset_readiness_path": str(figma_data_dir / ASSET_READINESS_NAME),
            "figma_proxy_configured": bool(figma_proxy and figma_proxy.strip()),
        },
