...
- Keep helper components and extracted assets next to the primary output unless the project already uses a different local pattern.
...
- Start each visible region by asking which `Ant Design 5` primitive should own it. Prefer `Flex`, `Typography`, `Space`, `Card`, `Layout`, `Button`, `Input`, `Tabs`, `Tag`, `Badge`, `Avatar`, and `Image` before falling back to raw wrappers.
- Use `Tailwind` utility classes and arbitrary values to match spacing, width, height, padding, margin, flex behavior, grid behavior, borders, shadows, radii, opacity, and responsive rules exactly.
- Do not create `.css`, `.scss`, `.sass`, `.less`, CSS Modules, styled-components, emotion styles, or `<style>` blocks for generated output.
- Keep authored styling in `className` and Ant Design props. Prefer `Tailwind` arbitrary values over falling back to a stylesheet.
- Do not create shared `CSSProperties` style-token objects for ordinary styling. Express ordinary typography, spacing, and color styling in `className`.
- Do not create shared static `Tailwind` class-string constants such as `const navMutedTextClass = '...'` for ordinary styling. Prefer direct string literals in JSX and string props.
- Use inline `style` only when `Tailwind` cannot express the needed value cleanly, or when the value is genuinely data-driven at runtime.
- Emit integer `px` values in code. Round long Figma decimals to the nearest practical pixel before generating `className` or `style`.
...
- Prefer concise static React. Avoid unnecessary state, effects, context, memoization, helper hooks, or extra wrapper components when the Figma output is presentational.
- Keep the component tree shallow and readable. Favor inline section markup or local typed arrays over one-file-per-layer decomposition.
...
- `Tailwind` utility classes for visual styling and layout with no sidecar stylesheet
- local typed data arrays or tiny helper functions only when they improve clarity without hiding the Figma geometry
...
- no generated custom stylesheet file or style block was needed to achieve the result
- the generated React stays concise and avoids unnecessary hooks, state, and over-abstraction
- ordinary styling lives in `Tailwind` classes rather than reusable `CSSProperties` style objects
- ordinary static `Tailwind` class strings stay inline in JSX or direct string props rather than shared `const ...Class` variables
- pixel values in code are integer `px` values rather than long fractional decimals
