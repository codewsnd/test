...
- Start each major visible region with `Ant Design 5` structure such as `Flex`, `Typography`, `Space`, `Card`, `Layout`, `Button`, `Input`, `Tabs`, `Tag`, `Badge`, `Avatar`, or `Image` before reaching for raw wrappers.
- Use `Tailwind` to match the Figma geometry, spacing, alignment, radii, borders, shadows, and responsive behavior exactly.
- Keep ordinary styling in `Tailwind` classes. Use inline `style` only when `Tailwind` cannot express the value cleanly or when the value is runtime-driven.
- Keep ordinary static `Tailwind` strings inline in JSX or direct string props instead of extracting `const ...Class = '...'` helpers.
- Round Figma subpixel values to integer `px` values in generated code.
...
- Prefer small typed data arrays and local `.map()` rendering over many tiny one-off wrapper components when the design is static.
...
- Avoid creating leaf components that only restate a single static wrapper with no real reuse or behavioral value.
...
- Keep file structure shallow and local, but do not extract ordinary static Tailwind class strings into shared constants for generated output.
- Prefer a few meaningful files and a shallow component tree over a literal file-per-layer translation of the Figma hierarchy.
