...
- Confirm the generated output does not rely on an authored custom stylesheet or style block.
- Confirm ordinary styling is not stored in reusable `CSSProperties` style-token objects.
- Confirm ordinary static `Tailwind` class strings are not hidden behind shared `const ...Class` variables unless a real reuse boundary clearly justifies it.
- Confirm code uses integer `px` values rather than long fractional pixel literals.
...
- Confirm that major sections, showcases, nav blocks, and footers are not represented by one large image unless the design region is itself a single asset.
