...
- [Strict Test Constraints](#strict-test-constraints)
...
5. Source async boundaries that can be replaced with synchronous mocks
...
- Test bodies must be synchronous: no `async`, `await`, `done`, `Promise`, `setTimeout`, or `setInterval`.
- Do not use `findBy*`, `waitFor`, or `@testing-library/user-event`.
- Trigger interactions only with `fireEvent`.
- Mock every external boundary with `vi.mock()`: child components, hooks, stores, router modules, request clients, network calls, browser APIs, and side-effect utilities.
- Keep mocks tiny. Use the shortest fixture that can execute the target branch.
- Do not assert `className`, inline `style`, computed style, or CSS properties.
- Assert rendered content, conditional branches, pure function outputs, and event callback payloads.
- Combine related synchronous checks in one test when it reduces setup without obscuring the failing branch.
...
- Never use `done`.
- Never depend on real sleeping, delayed retries, fake timers, or timer flushing.
- Keep network, router, browser API, and observer mocks synchronous and local.
- Mock debounce, throttle, polling, retry, delayed Promise, animation-frame, upload, download, and timer-backed utilities at the module boundary.
